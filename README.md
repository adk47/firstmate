...
| `/roster`          | Print one read-only matrix of every agent firstmate supervises - what it is doing now, the model it runs on, its context use, and an expected completion whose basis is stated - then the same for everything still running without supervision |
...
- [docs/deepseek-lane-gateway.md](docs/deepseek-lane-gateway.md) - the second local gateway that serves DeepSeek V4.1 Flash to opt-in Claude Code lanes, the measured proof sequence, cost per million, and the rollout order.
