...
`pool_authority` names which source decided the counts, because that is what says whether a torn file decided anything.
While it reads `inventory` and `pool_unreadable` names an account, the whole poll is could-not-determine and changes nothing: no failover episode opens, none closes - a tear is not a recovery, and resolving on one would ring the seat again the moment the file is readable - the recorded capable and tracked sets stand, the torn account keeps the needs-authentication membership it was last observed to have, and neither an entrance nor a `capacity back account=` label is emitted for it.
The `RED` and the wake still happen; only the doorbell waits for a clean read.
Once the pool is `GREEN` or `YELLOW` an open episode may still be resolved on such a poll: one more live account cannot empty a capable set or zero a routable count, and holding the marker across a file that never heals would block every later episode.
While `pool_authority` reads `ccflare` the inventory decided nothing the verdict rests on, so a torn file there is reported on the line and gates none of this.
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates, unless its own crew declared the wait, which takes the FM_PAUSE_RESURFACE_SECS recheck below instead; stale panes whose crew is not provably working surface immediately unless admitted directly to the declared-wait cadence, while a live idle declared wait still surfaces once before that cadence bounds repeats
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds between bounded rechecks of a declared external wait or verified captain-held transfer, including a live idle pane after its first inconclusive stale wake, a provably-working idle pane whose crew declared the wait, and a live busy pane past FM_BUSY_TURN_MAX_SECS; the away-mode daemon uses the same setting, ageing its window against the crew's own latest status line rather than pane busy state
