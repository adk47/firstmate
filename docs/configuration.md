...
The optional pool is the one exception: a home without `better-ccflare` is a normal firstmate home, so a pool that cannot be read is `UNKNOWN` and leaves the overall state to the supervisor's own runway.
