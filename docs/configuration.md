...
Neither observation-based failover episode opens while `pool_unreadable` names anything: the `RED` and the wake still happen, and only the doorbell waits for a clean poll.
