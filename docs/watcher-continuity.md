...
While a main follow-up is accepted but still unconsumed, a later actionable close folds under it instead of queueing a second follow-up, because one drain presents every queued durable row and the second prompt would only repeat that doorbell; consumption finishes every folded record, and a close carrying typed failure detail is never folded.
Pi emits `agent_settled` only once a run has fully settled and no automatic retry, compaction, or queued continuation will still drain its follow-up queue (an error or abort emits `agent_end` before any of those run), so a follow-up still unconsumed at `agent_settled` was dropped from that queue (Escape, dequeue) and can never be consumed; after a short grace (`FM_PI_WAKE_FOLD_GRACE_MS`, default 3000 ms) the extension releases it without finishing its records, and the pipeline redelivers the first as a fresh follow-up with the rest folded under it, so a dropped doorbell never silences later closes.
