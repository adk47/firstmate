...
1. **Detect.** The actor that reads the idle agent's pane classifies its tail and opens a durable stall record on the first sighting.
   ...
   A pane that reads busy is that same evidence mid-turn and drops the record too, so a record never outlives the stall it records however long the recovered turn runs.
