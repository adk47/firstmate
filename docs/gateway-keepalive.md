...
`tests/fm-watch-triage.test.sh` drives a real watcher over a stalled pane and asserts the continue instruction lands in the steering inbox as a fire-and-forget record without a wake, that a spent budget declares the external wait and stops re-ringing, that an ordinary steer already waiting when the stall began is neither rung nor counted by the inbox ladder while the stall record is open - through the re-ring and through the declared wait after the budget is spent - and is rung once the record drops, that a window whose pane cannot be captured still has its unread steer escalated, that the harness's own internal 503 retry keeps the record and still reaches a declared outage while a busy stretch reaching the threshold drops it so a later transient error starts a fresh ladder, and that a paused secondmate whose pane shows the error is re-surfaced as a paused mate and never re-rung.
