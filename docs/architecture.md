...
An idle pane whose run step still reads as actively working takes that same declared-wait recheck instead of the wedge timer, because the declaration identifies the static step as the expected wait; a worker with no declaration keeps the unchanged wedge reason, timing, and escalation count.
...
An idle crewmate or scout pane whose own turn-lifecycle record says its last turn ended on a transient inference-gateway error is absorbed and re-rung ahead of all of that stale classification, and only a spent re-ring budget hands it back, as that crew's own declared wait; [`gateway-keepalive.md`](gateway-keepalive.md) owns that ladder.
...
`bin/fm-fleet-snapshot.sh` folds each task's durable open-decision set through that same cursor-backed scan, run inside its concurrent per-task prefetch and folded only up to the end of the captured status copy so the hints and current state share one observation; that keeps the snapshot's fold bounded by new appends too, reading the one `state/.<id>.open-decisions-cursor` the drain owns rather than re-reading each whole log, and it folds read-only, never advances that cursor because the cursor doubles as the drain's legacy presentation offset, and keeps the fold's scratch chunk in its own temp dir so a snapshot killed at its deadline leaves nothing behind in `state/` ([`fm-classify-lib.sh`](../bin/fm-classify-lib.sh)'s fold header owns the cursor format, invalidation, and read-only option).
Every latest-line read (`last_status_line`, and the snapshot's `last_nonempty_line`) inspects at most the final 64 KiB of an append-only log and falls back to a whole-file scan only when that window holds no complete non-blank line, so no per-task read in the supervision path scales with a log's lifetime size; `tests/fm-status-log-bounded-read.test.sh` pins that bound and the snapshot fold's read-only, captured-end behaviour on recorded byte counts.
...
A keyed `needs-decision` or `blocked` transition is actionable only while it is still the live opening for its key, and the classifier decides that by folding just the appended span rather than re-reading the whole log: the span always runs to the file's end, so a later same-key `resolved`, `captain-held`, or reopening inside it retires the earlier opening exactly as the whole-file fold would, while a reserved-key transition the fold rejects surfaces as a reconciliation signal without becoming an open decision.
`tests/fm-classify-fold-bounded.test.sh` pins that bounded cost for both the span classifier and the cursor-backed OPEN DECISIONS fold, because a multi-megabyte append-only status log is a normal shape for a long-lived monitoring lane and a whole-log re-read per poll is what once let the watcher's liveness beacon go stale while it was still working.
...
An H-DevOps pull request derives its targets from the Deployment documents its diff hunks land in across the changed manifests under `k8s/prod/`, read at the PR's head commit, under each document's own `metadata.namespace` (a Deployment declaring none is skipped with its reason), and falls back to the whole manifest, saying so, only when a hunk cannot be placed; an application repository resolves its name to the Deployment that carries it in the local H-DevOps checkout; a PR that touches no deployable service registers nothing.
...
Each sample reads the Deployment's own signals - 5xx rate, pod restarts, OOMKilled events, p95 latency where the service exposes it, waits on the Deployment's own pgbouncer pool, and container working set - and compares each to its baseline with a stated bar.
A metric that cannot be read is `unmeasured` and is skipped rather than scored as a pass, so an outage of the measurement itself never reads as health: a schedule that ends with a target never read completes `unmeasured`, not clean, and a drive stopped before the end of its schedule records `interrupted at +Nm`, queues one check wake, and is re-armed for its remaining samples by registering it again.
