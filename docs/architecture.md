...
An H-DevOps pull request derives its targets from the Deployment documents its diff hunks land in across the changed manifests under `k8s/prod/`, under each document's own `metadata.namespace`, and falls back to the whole manifest, saying so, only when a hunk cannot be placed; an application repository resolves its name to the Deployment that carries it in the local H-DevOps checkout; a PR that touches no deployable service registers nothing.
