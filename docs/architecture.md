...
`bin/fm-fleet-snapshot.sh` folds each task's durable open-decision set through that same cursor-backed scan, run inside its concurrent per-task prefetch, so the snapshot's fold is bounded by new appends too and reads the one `state/.<id>.open-decisions-cursor` the drain owns rather than re-reading each whole log; it folds read-only, never advances that cursor because the cursor doubles as the drain's legacy presentation offset, and keeps the fold's scratch chunk in its own temp dir so a snapshot killed at its deadline leaves nothing behind in `state/` ([`fm-classify-lib.sh`](../bin/fm-classify-lib.sh)'s fold header owns the cursor format, invalidation, and read-only option).
