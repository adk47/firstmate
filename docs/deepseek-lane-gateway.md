...
- **The port is gated by a local token.** Everything except `/healthz` requires the bearer token in `$STATE/fm-deepseek-gateway.token` (mode 0600, created on first use).
  ...
  The route itself lives on `GET /status`, behind the token: the resolved provider, slot, upstream model, base URL, `key_present`, a `route_ok` boolean, and the picker's own message, which is an unowned script's stderr and so is never served unauthenticated. That resolution is cached for 10 seconds - the server's own `--route-ttl`, which `start` does not forward - so polling `status` does not fork the picker per request; request routing is unaffected and still resolves per request.
