...
FM_TEST_ORCA_JSON=$(orca_json pi "π - firstmate")
out=$(FM_TEST_CMDLINE='pi' FM_CHAIR_STATUS_TTY_CMD=true run_status)
assert_contains "$out" "terminal=term_x" "matching terminal accepted"
pass "matching title and harness selects the terminal"

# --- context-full pi counts as no chair -------------------------------------

FM_TEST_ORCA_JSON=$(orca_json pi "π - firstmate" true "context 100% used")
out=$(FM_TEST_CMDLINE='pi' run_status)
assert_contains "$out" "chair=none" "a 100%-context pi chair is no chair"
assert_contains "$out" "reason=context_full" "context-full reason"
pass "100%-context pi footer -> none"

# --- json -------------------------------------------------------------------

FM_TEST_ORCA_JSON=$(orca_json grok "grok - firstmate")
out=$(FM_TEST_CMDLINE='grok' run_status --json)
printf '%s' "$out" | jq -e '.chair == "grok"' >/dev/null || fail "json chair"
printf '%s' "$out" | jq -e '.terminal == "term_x"' >/dev/null || fail "json terminal"
pass "json output carries chair and terminal"
