...
config/fable-runway.env  optional FM_FABLE_RUNWAY_GROK_TERMINAL handle the Fable-runway failover doorbell rings; parsed, never sourced; LOCAL, gitignored, and NOT inherited by secondmate homes; see docs/configuration.md "Fable runway monitor"
config/sentry-watch.json  optional Sentry watch thresholds, signatures, per-project overrides, and rail settings read by the check armed with bin/fm-sentry-watch.sh; absent means built-in defaults; LOCAL, gitignored, and NOT inherited by secondmate homes; see docs/configuration.md "Sentry watch"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  handoff-<from>-to-<to>.md  memory carried across a firstmate chair flip (sensor line, chosen source, in-flight task tails, memory pointers); written by bin/fm-chair-flip.sh before the incumbent is ended and named in the successor's first prompt
  chair-sentinel/    one JSONL decision line per chair-sentinel tick plus the LaunchAgent's stdout/stderr; bin/fm-chair-sentinel.sh owns the format
...
state/               runtime records and signals; gitignored
  ...
  <id>.gateway-stall  durable record of an agent stalled on a transient inference-gateway failure and its bounded re-ring budget; bin/fm-gateway-retry-lib.sh owns the format, teardown removes it (docs/gateway-keepalive.md)
  ...
  fable-runway.check.sh  generated Fable-runway poll shim and its .check-trust binding; present only after bin/fm-fable-runway-check.sh arm; its record .fable-runway, the once-per-episode marker .fable-runway-handoff, and the captain's handoff notes fable-runway-handoff-<epoch>.md are owned by that check and bin/fm-fable-runway-alert.sh (docs/configuration.md "Fable runway monitor")
  sentry-watch.check.sh  generated Sentry watch poll shim and its .check-trust binding; present only after bin/fm-sentry-watch.sh arm; its beat .sentry-watch.beat, armed marker .sentry-watch.armed, and rail record .sentry-watch.rail.json go with disarm, while the shared seen-set sentry-watch.baseline.json survives disarm because the Sentry mobile scout reads it (docs/configuration.md "Sentry watch")
  ...
  .hash-* .count-* .stale-* .stale-since-* .churn-since-* .paused-* .wedge-escalations-* .writing-* .gw-busy-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
  .watch-triage.log  watcher's absorbed-wake debug log (size-capped); never relied on, safe to delete
  .last-watcher-beat watcher liveness beacon, touched every poll (including while absorbing benign wakes) and between a slow poll's stages; guard scripts read it
  .chair-source .chair-flip-at .chair-alarm .chair-fable-unknown-ticks   firstmate chair sentinel records: the lock holder's bound token source, the flip hysteresis stamp, the no-tank alarm, and the consecutive Fable-unmeasured count; bin/fm-chair-flip.sh and bin/fm-chair-sentinel.sh own the formats; never touch
...
3. **Wake queue** - when locked, drains and presents the durable wake queue without running the inactive-outcome scan inline, and prints the raw records prominently as this turn's first work queue; a clearly labeled status-event annotation may follow a valid `signal` record and includes every status line still unread at the presentation cursor, but never replaces the raw record or current-state reconciliation, and a lapsed watcher chain still surfaces here via the same guard alarm.
   ...
   Ahead of those sections it prints one `SENTRY WATCH DARK` line when `state/.sentry-watch.armed` says this home watches Sentry but the check, its trust binding, or its beat is missing or stale; Sentry visibility is lost until you re-arm it as that line says (docs/configuration.md "Sentry watch").
...
7. **Context digest and next step** - last of the bulk sections, the full contents of `data/projects.md`, `data/secondmates.md`, `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, each clearly delimited, followed by the closing reminder.
   ...
   The closing reminder points back to the emitted supervision block and preserves only the lock, afk, Relay, and read-once reminders, then the digest ends with the house-wiki stanza (section 6 owns that contract).
...
An agent left idle by a transient inference-gateway failure is re-rung automatically with a bounded continue instruction rather than surfaced as a wedge, so a continue message you did not send is expected; only a spent budget surfaces, as that agent's own `paused [key=gateway-503]` declared wait ([`docs/gateway-keepalive.md`](docs/gateway-keepalive.md)).
...
- `change-watch` - load on a `check: change-watch <task-id> <pr-url> <metric> regressed` wake, on a `check: change-watch <task-id> <pr-url> interrupted at +Nm` wake, on a `procevent when-cw-... <sequence>` completion wake for an armed post-merge change watch, or when deciding whether a merged change needs a by-effect watch.
