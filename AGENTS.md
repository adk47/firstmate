...
state/               runtime records and signals; gitignored
  ...
  <id>.gateway-stall  durable record of an agent stalled on a transient inference-gateway failure and its bounded re-ring budget; bin/fm-gateway-retry-lib.sh owns the format, teardown removes it (docs/gateway-keepalive.md); the primary session's own stall is the same record scoped .primary.gateway-stall, which no teardown owns
  .primary-endpoint .keepalive-agent.log  this home's own recorded primary terminal endpoint, refreshed by the main home's session start (bin/fm-bootstrap.sh primary_keepalive_setup, opted out by config/keepalive-off; secondmate homes opt in through bin/fm-keepalive-install.sh), and the out-of-session keep-alive job's bounded log; written only by bin/fm-keepalive-endpoint.sh and bin/fm-keepalive-agent.sh
  .keepalive-uninstalled  why this home's primary has no working keep-alive, reported once per session start as an actionable KEEPALIVE line and cleared when the condition lifts; bin/fm-gateway-retry-lib.sh owns the format, safe to delete
  .keepalive-agent.lock .keepalive-session-gone .keepalive-supervision-nagged  the keep-alive agent's single-flight lock directory and its two nag throttles; bin/fm-keepalive-agent.sh owns them, never touch while the job may be running
  .keepalive-agent.out .keepalive-agent.err  the launchd job's raw stdout and stderr sinks, named by the plist bin/fm-keepalive-install.sh writes; nothing reads them, safe to delete
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `NETWORK_CHECKS:`, `HOME_SUMMARY:`, `BACKLOG_RECONCILE:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, `KEEPALIVE:`, or `FMX:`), or when `BOOTSTRAP_INFO:` says an interrupted backlog cleanup may have left an endpoint or local copy; silence and other `BOOTSTRAP_INFO:` facts need no load.
