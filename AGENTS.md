...
- `change-watch` - load on a `check: change-watch <task-id> <pr-url> <metric> regressed` wake, on a `procevent when-cw-... <sequence>` completion wake for an armed post-merge change watch, or when deciding whether a merged change needs a by-effect watch.
